        ...
        <script>
            ...
            $(function(){
                ...
                function initGroupOptions() {
                    ...
                    // ✅ 修改：传 gse/gsm
                    $.getJSON(ctx + '/deg', { said: said, gse: gse, gsm: gsm, pval: 1.0, fc: 0.0 })
                        .done(function(data){
                            // 如果 servlet 返回的是 {"error":...} 也兜底一下
                            if (data && data.error) {
                                console.error("❌ DEG error:", data.error, data.path || "");
                                alert("DEG load failed: " + data.error + (data.path ? ("\n" + data.path) : ""));
                                return;
                            }

                            console.log("✅ DEG group data fetched");
                            const groups = Array.from(new Set(
                                (data || []).map(r => typeof r.group === "string" ? r.group.trim() : null).filter(g => g)
                            ));
                        ...
                        .fail(function(xhr){
                            console.error("❌ DEG group data failed:", xhr.status, xhr.statusText);
                            alert("DEG group request failed: " + xhr.status + " " + xhr.statusText);
                        });
                }
                ...
                function loadDEG(){
                    ...
                    // ✅ 修改：传 gse/gsm
                    const params = { said: said, gse: gse, gsm: gsm, pval: pval, fc: fc };
                    if (group) params.group = group;

                    console.log("📡 Requesting DEG data:", params);
                    $.getJSON(ctx + '/deg', params)
                        .done(function(data){
                            if (data && data.error) {
                                console.error("❌ DEG error:", data.error, data.path || "");
                                table.clear().draw();
                        ...
                        .fail(function(xhr){
                            console.error("❌ DEG data loading failed:", xhr.status, xhr.statusText);
                            table.clear().draw();
                            alert("DEG request failed: " + xhr.status + " " + xhr.statusText);
                        });
                }

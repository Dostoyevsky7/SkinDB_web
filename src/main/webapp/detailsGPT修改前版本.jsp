...
<%
    ...
    if (gseForAction == null || gseForAction.trim().isEmpty()) {
        gseForAction = "GSE130973";
    }
    if (gsmForAction == null || gsmForAction.trim().isEmpty()) {
        gsmForAction = "GSM3758115";
    }

    String cpdbPath = application.getRealPath(
            "/SkinDB_New/10X/human/" + gseForAction + "/" + gsmForAction + "/cpdb_out"
    );
    String outputFileName = "plot.png";
    String scriptPath = new File(cpdbPath, "run_cpdb_plot.py").getAbsolutePath();

    outPage.println("<html><head><title>Details for " + gsmForAction + "</title></head><body>");
    outPage.println("<h2>Details for GSE: " + gseForAction + " | GSM: " + gsmForAction + "</h2>");

    try {
        File workingDir = new File(cpdbPath);
        if (!workingDir.exists() || !workingDir.isDirectory()) {
            outPage.println("<p style='color:red;'>❌ 工作目录不存在: " + cpdbPath + "</p>");
        } else {
            String[] command = {PYTHON_COMMAND, scriptPath, gsmForAction};
            ...
            if (exitCode == 0) {
                ...
                if (outputFile.exists()) {
                    String imageUrl = request.getContextPath()
                            + "/SkinDB_New/10X/human/"
                            + gseForAction + "/" + gsmForAction + "/cpdb_out/" + outputFileName;

                    outPage.println("<p>✅ 分析成功！结果如下：</p>");
                    outPage.println("<img src='" + imageUrl + "' alt='Plot Result' style='max-width:800px;'>");
                } else {
                    outPage.println("<p style='color:red;'>⚠️ 分析完成但未找到结果图文件: " + outputFileName + "</p>");
                }
            } else {
                outPage.println("<p style='color:red;'>❌ Python 脚本执行失败 (exitCode=" + exitCode + ")</p>");
                outPage.println("<pre>" + output.toString() + "</pre>");
            }
        }
    } catch (Exception e) {
        outPage.println("<p style='color:red;'>❌ 异常: " + e.getMessage() + "</p>");
    }

    outPage.println("<hr><a href='index.jsp'>返回列表</a>");
    outPage.println("</body></html>");
%>


<!-- ========== SECTION B: JSP 原有 HTML 展示逻辑（不会执行，因为 return 已经结束） ========== -->
